function solveMF1_2b( q_x, k, beta, gamma1, gamma2, f_x, l )
%solve following boundary value problem:
% k*u''(x)-q(x)*u(x) = -f(x), 0<x<l
% -k*u'(0)+beta*u(0)=gamma1, u(l)=gamma2
%using method of fictitious point
%
%usage:
% solveMF1_2b( q_x, k, beta, gamma1, gamma2, f_x, l )
    ...
    % f - vector of free members
    f = zeros(1,N);
    f(1) = gamma1 + h*f_x(x(1));
    for i = 2:N-1
        f(i) = -f_x(x(i));
    end;
    f(N) = gamma2;
